...
/*
 * 布局说明：
        <com.chenliang.library.view.MyRefreshListView
...
class MyRefreshListView : SmartRefreshLayout {


    private var rootView: FrameLayout? = null
    private var emptyLayout: View? = null
    private var recyclerView: MyListVIew? = null
    ...
    constructor(context: Context?, attributeSet: AttributeSet) : super(context!!, attributeSet) {
        val typedArray =
            context.obtainStyledAttributes(attributeSet, R.styleable.MyRefreshListView)
        emptyLayoutId = typedArray.getResourceId(R.styleable.MyRefreshRecyclerView_empty_layout, 0)
        layoutId = typedArray.getResourceId(R.styleable.MyRefreshListView_item, 0)
        params = SmartRefreshLayout.LayoutParams(
    ...
    private fun initRecyclerView(context: Context?, attributeSet: AttributeSet) {
        recyclerView = MyListVIew(context!!, attributeSet)
    ...
    public fun <T : Any> observeData(mutableLiveData: MutableLiveData<ArrayList<T>>) {
